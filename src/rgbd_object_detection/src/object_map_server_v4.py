...
class ObjectMap:
    ...
    def publish_pedestrian_staff_marker_position_callback(self, data):
        if (self.human_number_array != []):
            ...
            for p_marker in data.markers[0].points:
                ...
                pedestrian_staff_marker.id = self.human_number_array[count]
                pedestrian_staff_marker.type = Marker.CUBE_LIST
                pedestrian_staff_marker.scale.x = 0.5
                pedestrian_staff_marker.scale.y = 0.5
                pedestrian_staff_marker.scale.z = 1.2
                pedestrian_staff_marker.color.b = 1.0
                ...
                pedestrian_staff_text.id = self.human_number_array[count]
                ...
                pedestrian_staff_text.pose.position.z = pos_in_map.pose.position.z + 0.7
                ...
                if (self.human_number_array[count] <= 2):
                    pedestrian_staff_text.text = 'Pedestrian_'+str(self.human_number_array[count])
                else:
                    pedestrian_staff_text.text = 'Staff_'+str(self.human_number_array[count])
    ...
    def contact_time_and_distance_callback(self, data):
        ...
        for staff_idx in range(len(self.object_map_)):
            if self.object_map_[staff_idx].occupied_person_leave_ == True:
                polygon = shapely.geometry.Polygon(self.object_map_[staff_idx].points_)
                person_count = 1
                for person_m in data.markers[0].points:
                    if data.markers[person_count].id > 2:
                        ...
                        if (object_staff_distance < self.dist_thresh_):
                            self.object_map_[staff_idx].set_disinfecting()
                        elif (object_staff_distance >= self.dist_thresh_) and (self.object_map_[staff_idx].disinfecting_ == True):
                            self.object_map_[staff_idx].disinfecting_ = False
                            self.object_map_[staff_idx].occupied_person_leave_ = False
                            self.object_map_[staff_idx].need_clean_ = False
                    person_count+=1
