...
class ObjectMap:
    ...
    def processFeedback (self, feedback):
        if feedback.event_type == InteractiveMarkerFeedback.BUTTON_CLICK:
            # print(self.object_map_[i].occupied_person_leave_)
            # print(self.object_map_[i].need_clean_)

            m_x = feedback.mouse_point.x
            m_y = feedback.mouse_point.y
            
            object_idx = -1
            default_distance = 10000.000
            for i in range (len(self.object_map_)):
                polygon = self.object_map_[i].points_
                poly_x = 0
                poly_y = 0
                for idx in range (len(polygon)):
                    poly_x = poly_x + polygon[idx][0]
                    poly_y = poly_y + polygon[idx][1]
                poly_x = poly_x/len(polygon)
                poly_y = poly_y/len(polygon)
                
                d = (poly_x-m_x)**2+(poly_y-m_y)**2
                objecti_distance = round(sqrt(d), 3)
                print("--------------------------")
                # print("objecti_distance: ", objecti_distance)

                if(objecti_distance < default_distance):
                   default_distance = objecti_distance
                   object_idx = i
                #    print("hi: ", object_idx)
            
            print("idx: ",object_idx)
            self.object_map_[object_idx].occupied_person_leave_ = False
            self.object_map_[object_idx].need_clean_ = False
        ...
        if len(point_z) != 0:
            for i in range(len(self.object_map_)):
                ...
                for j in range(self.object_map_[i].points_.shape[0]):
                    point = Point()
                    point.x = round(self.object_map_[i].points_[j, 0], 6)
                    point.y = round(self.object_map_[i].points_[j, 1], 6)
                    point.z = 0
                ...
                point = Point()
                point.x = round(self.object_map_[i].points_[0, 0], 6)
                point.y = round(self.object_map_[i].points_[0, 1], 6)
                point.z = 0
                line_strip.points.append(point)
                ...
                if self.object_map_[i].occupied_person_leave_ == True:
                    ...
                    line_list.color.g = 1.0
                    line_list.color.r = self.object_map_[i].occupied_ / float(self.need_clean_thresh_)
                else:
                    line_list.color.g = 1.0
                ...
                for b in box:
                    ...
                    point_point.append(round(b[0],3))
                    point_point.append(round(b[1],3))
                    point_point.append(round(point_z[i],3))
                ...
                # x_inter = (box[2][0]+box[1][0])/2
                # y_inter = (box[0][1]+box[1][1])/2
                # # print(box)
                # # print(x_inter, y_inter)
                # z_inter = (point_z[i])/2
                # marker_interactive.pose.position.x = x_inter
                # marker_interactive.pose.position.y = y_inter
                # marker_interactive.pose.position.z = z_inter
                ...
                control.markers.append(line_list)
                control.markers.append(class_tag)
                ...
                marker_interactive.controls.append(control)

            server.insert(marker_interactive, self.processFeedback)
            server.applyChanges()

            self.interactive_result_pub_.publish(marker_interactive)
    ...
    def people_callback(self, data):
        ...
        for idx in range(len(self.object_map_)):
            if self.object_map_[idx].need_clean_ == True:
                polygon = shapely.geometry.Polygon(self.object_map_[idx].points_)
                for person in person_list:
                    distance = person.distance(polygon)
                    if distance >= self.dist_thresh_:
                        self.object_map_[idx].set_occupied_person_leave()
