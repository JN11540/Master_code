...
class ObjectMap:
    ...
    def __init__(self):
        ...
        self.dist_thresh_ = 0.5 # the distance from a person to an object for the object to be considered as occupied
    ...
    def publish_convex_hull_marker(self):
        ...
        if len(point_z) != 0:
            for i_o in range(len(self.object_map_)):
                ...
                if [x_inter, y_inter] not in box_centroid_array:
                    box_centroid_array.append([x_inter, y_inter])
                # print(len(box_centroid_array))
    ...
    def contact_time_and_distance_callback(self, data):
        ...
        for person_marker in data.markers[0].points:
            ...
            person_coordinates = [round(pos_in_map.pose.position.x, 3), round(pos_in_map.pose.position.y, 3)]
            person_list.append(person_coordinates)
        ...
        for obj_idx in range(len(self.object_map_)):
            ...
            # 获取中心点的坐标
            centroid_x = round(box_centroid_array[obj_idx][0], 3)
            centroid_y = round(box_centroid_array[obj_idx][1], 3)

            # print(centroid_x, centroid_y)

            if_occupied = False
            for person_coordinates in person_list:
                x_squared = (person_coordinates[0] - centroid_x)*(person_coordinates[0] - centroid_x)
                y_squared = (person_coordinates[1] - centroid_y)*(person_coordinates[1] - centroid_y)
        ...
        for idx_leave in range(len(self.object_map_)):
            if self.object_map_[idx_leave].need_clean_ == True:
                ...
                c_x = round(box_centroid_array[idx_leave][0], 3)
                c_y = round(box_centroid_array[idx_leave][1], 3)

                for person in person_list:
                    x1_squared = (person[0] - c_x)*(person[0] - c_x)
                    y1_squared = (person[1] - c_y)*(person[1] - c_y)
                    distance_leave = math.sqrt(x1_squared+y1_squared)
                    # distance = person.distance(polygon)
                    if distance_leave >= self.dist_thresh_:
        ...
        for ob_idx in range(len(self.object_map_)):
            if self.object_map_[ob_idx].occupied_person_leave_ == True:
                ...
                cc_x = round(box_centroid_array[ob_idx][0], 3)
                cc_y = round(box_centroid_array[ob_idx][1], 3)

                person_count = 1
                for person_m in data.markers[0].points:

                    # transform to map frame
                    pos_in_velodyne1 = PoseStamped()
                    pos_in_velodyne1.pose.position.x = person_m.x
                    pos_in_velodyne1.pose.position.y = person_m.y
                    pos_in_velodyne1.header.frame_id = "velodyne"
                    pos_in_velodyne1.pose.orientation.w = 1.0
                    pos_in_map1 = self.tf_listener_.transformPose("map", pos_in_velodyne1)

                    # print(data.markers[person_count].text)
                    if "Staff_" in data.markers[person_count].text:
                        staff_position = [round(pos_in_map1.pose.position.x, 3), round(pos_in_map1.pose.position.y, 3)]
                        xx_squared = (staff_position[0] - cc_x)*(staff_position[0] - cc_x)
                        yy_squared = (staff_position[1] - cc_y)*(staff_position[1] - cc_y)
                        object_staff_distance = math.sqrt(xx_squared+yy_squared)
